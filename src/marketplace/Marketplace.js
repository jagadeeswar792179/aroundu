...
export default function Marketplace() {
  ...
  const fetchFeed = async ({ limit = 12, cursor, id }) => {
    ...
    if (!res.ok)
      throw new Error(
        data?.message || data?.msg || "Failed to load marketplace",
      );
    return data;
  };
  ...
  const fetchSearch = async ({ q, limit = 12, cursor, id }) => {
    ...
    const res = await fetch(
      `${API_BASE}/api/marketplace/search?${params.toString()}`,
      {
        method: "GET",
        headers: { ...getAuthHeader() },
      },
  ...
  const fetchMine = async ({ limit = 12, cursor, id }) => {
    ...
    const res = await fetch(
      `${API_BASE}/api/marketplace/mine/list?${params.toString()}`,
      {
        method: "GET",
        headers: { ...getAuthHeader() },
      },
    ...
    const data = await res.json();
    if (!res.ok)
      throw new Error(data?.message || data?.msg || "Failed to load my items");
    return data;
  };
      ...
      <div className="market-wrap">
        ...
        <div className="market-actions">
          ...
          <div className="market-actions-tab">
            <div
              className={`market-btn ${view === "feed" ? "market-button" : "market-btn-secondary"}`}
            >
              <svg
                ...
                >
                  ...
                  <div className="market-card-body">
                    ...
                    <div className="market-meta">
                      ...
                      <span>
                        {it.price
                          ? `${getCurrencySymbol(it.currency)} ${Number(it.price).toFixed(0)}`
                          : "Free"}
                      </span>
                    </div>
...
function ListingModal({
  ...
  const fallbackImages = item.thumbnail
    ? [{ presignedUrl: item.thumbnail }]
    : [];
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              ...
              <div>
                <div
                  ...
                  onClick={
                    !isMine
                      ? () => onNavigateProfile(item.seller.id)
                      : undefined
                  }
                >
        ...
        <div className="m-content">
              ...
              <div className="m-thumbs">
                {imgList.map((im, idx) => (
                  <img
                    ...
                    alt={`thumb-${idx}`}
                    style={{
                      outline: idx === active ? "2px solid #111" : "none",
                    }}
                    onClick={() => setActive(idx)}
                  />
                ))}
          ...
          <div className="m-right">
            ...
            <div className="m-price">
              {listing.price
                ? `${getCurrencySymbol(listing.currency)} ${Number(listing.price).toFixed(0)}`
                : "Free"}
            </div>

            <div className="m-time">
              <TimeAgo timestamp={listing.created_at} /> in{" "}
            ...
            {!isMine ? (
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                <button
                  className="market-btn market-btn-secondary"
                  onClick={() => onEditMine(listing)}
                >
                  Edit
                </button>
                <button
                  className="market-btn market-btn-secondary"
                  onClick={() => onDeleteMine(listing.id)}
                >
                  Delete
                </button>
              </div>
            ...
            <div className="m-row">
              <span className="m-label">Condition:</span>
            ...
            {listing._loadingDetails ? (
              <div style={{ color: "#666" }}>Loading full details...</div>
            ) : null}
            {listing._err ? (
              <div style={{ color: "#a11" }}>{listing._err}</div>
            ) : null}
          </div>
        </div>
      </div>
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (mode === "edit" && initialData?.id) {
        const res = await fetch(
          `${API_BASE}/api/marketplace/${initialData.id}`,
          {
            method: "PUT",
            headers: {
              ...getAuthHeader(),
      ...
      for (const img of images) {
        const fileToUpload = img.editedFile
          ? img.editedFile
          : await compressWithoutCrop(img.file, {
              maxSize: 1600,
              quality: 0.85,
            });
      ...
      const data = await res.json();
      if (!res.ok)
        throw new Error(data?.message || data?.msg || "Failed to post item");
        ...
        <div className="p-form">
          ...
          {mode === "create" ? (
                        ...
                        <div style={{ display: "flex", gap: 6 }}>
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (img.editedPreviewUrl)
                                URL.revokeObjectURL(img.editedPreviewUrl);
                              setImages((prev) =>
            ...
            <div style={{ color: "#666", fontSize: 13 }}>
              Images are not editable in Edit mode (only fields). If you want
              image editing on edit, tell me — we’ll add it.
            </div>
          )}
          ...
          {editIndex !== null ? (
            <EditImageModal
              src={
                images[editIndex]?.editedPreviewUrl ||
                images[editIndex]?.previewUrl
              }
...
async function compressWithoutCrop(
  file,
  { maxSize = 1600, quality = 0.85, mimeType = "image/jpeg" } = {},
) {
...
function EditImageModal({ src, onClose, onSave }) {
  ...
  const save = async () => {
    ...
    const edited = new File([blob], `market-${Date.now()}.jpg`, {
      type: "image/jpeg",
    });
    onSave(edited);
  };
        ...
        <div style={{ padding: 12 }}>
          ...
          >
            ...
            <div className="p-actions">
              ...
              <button className="market-btn market-btn-primary" onClick={save}>
                Save
              </button>
            </div>
          </div>
