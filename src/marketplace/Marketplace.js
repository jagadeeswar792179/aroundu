...
//   // ---- API: listing details (carousel needs this)
...
//     });
//     const data = await res.json();
//     if (!res.ok) throw new Error(data?.message || data?.msg || "Failed to load listing");
//     return data; // { listing, images, seller }
//   };
...
//   // ---------- FIRST LOAD + WHEN SEARCH CHANGES ----------
//   useEffect(() => {
//     let mounted = true;
...
// const data = isMine
//   ? await fetchMine({ limit: 12 })
//   : isSearching
//     ? await fetchSearch({ q: debounced, limit: 12 })
//     : await fetchFeed({ limit: 12 });
//         if (!mounted) return;

//         setItems(data.items || []);
//         setNextCursor(data.nextCursor || null);

//         setHasMore(!!data.nextCursor && (data.items || []).length >= 12);
//       } catch (e) {
//         if (!mounted) return;
//         setErr(e.message || "Something went wrong");
//       } finally {
//         if (mounted) setLoadingFirst(false);
//       }
//     })();
...
//       setNextCursor(data.nextCursor || null);
//       setHasMore(!!data.nextCursor && newItems.length >= 12);
//     } catch (e) {
//       setHasMore(false);
...
//     try {
//       setSelected({ ...item, _loadingDetails: true });

//       const details = await fetchListingDetails(item.id);
...
//     } catch (e) {
//       setSelected((prev) =>
//         prev ? { ...prev, _loadingDetails: false, _err: e.message } : prev
//       );
//     }
//   };
...
//   <button
//     className={`market-btn ${view === "feed" ? "market-btn-primary" : "market-btn-secondary"}`}
//     onClick={() => setView("feed")}
//   >
//     Marketplace
//   </button>
//   <button
//     className={`market-btn ${view === "mine" ? "market-btn-primary" : "market-btn-secondary"}`}
//     onClick={() => setView("mine")}
//   >
//     My Items
//   </button>
// </div>
//             <button className="market-btn" onClick={() => setOpenPost(true)}>
//               Post Item
//             </button>
//           </div>

//           {err ? <div className="market-err">{err}</div> : null}

//           {loadingFirst ? (
//             <div style={{ padding: 12 }}>Loading items...</div>
//           ) : items.length === 0 ? (
//             <div style={{ padding: 12, color: "#666" }}>
//               {debounced ? "No results found." : "No items yet. Be the first to post."}
//             </div>
//           ) : (
//             <InfiniteScroll
...
//                 {items.map((it) => (
//                   <div key={it.id} className="market-card" onClick={() => openItem(it)}>
...
//                         <span>•</span>
//                         <span>
//                           {it.price ? `${it.currency} ${Number(it.price).toFixed(0)}` : "Free"}
//                         </span>
//                       </div>
...
//           {selected ? <ListingModal item={selected} onClose={() => setSelected(null)} /> : null}
...
//                     {item.seller?.profile ? (
//                       <img
//                         src={item.seller.profile}
//                         className="avatar-img"
//                         alt="profile"
//                       />
//                     ) : (
//                       <div className="avatar-fallback">
//                         {`${item.seller?.first_name?.[0] || ""}${
//                           item.seller?.last_name?.[0] || ""
//                         }`.toUpperCase()}
//                       </div>
//                     )}
//                     <div
...
//                         <b
//                           className={`username ${
//                             item.seller_id !== loggedInUserId ? "clickable" : ""
//                           }`}
//                           onClick={() => {
...
//                         <p>
//                            {item.seller?.role}{item.seller&&"at"} {item.seller?.university}
//                         </p>
//                         <p>{item.seller?.course}</p>
//                         {view === "mine" ? (
...
//               {activeUrl ? (
//                 <img className="m-main-img" src={activeUrl} alt={item.title} />
//               ) : (
//                 <div className="market-noimg">No image</div>
//               )}
...
//                 <>
//                   <button
//                     className={`m-nav m-nav-left ${!canPrev ? "m-nav-disabled" : ""}`}
//                     onClick={prev}
//                     disabled={!canPrev}
//                   >
//                     ‹
//                   </button>
//                   <button
//                     className={`m-nav m-nav-right ${!canNext ? "m-nav-disabled" : ""}`}
//                     onClick={next}
//                     disabled={!canNext}
//                   >
//                     ›
...
//           <div className="m-h-title">{item.title}</div>
            

//             <div className="m-price">
//               {item.price ? `${getCurrencySymbol(item.currency)} ${Number(item.price).toFixed(0)}` : "Free"}
//             </div>
//           <div className="m-time"><TimeAgo timestamp={item.created_at} /> in <span>{item.location || "-"}</span></div>

// <button
//                 onClick={() => setSelectedPeer(item.seller)}
...
//             <div className="m-row"><span className="m-label">Category:</span><span>{item.category || "-"}</span></div>
//             <div className="m-row"><span className="m-label">Condition:</span><span>{item.condition || "-"}</span></div>
...
//             {item._loadingDetails ? <div style={{ color: "#666" }}>Loading full details...</div> : null}
//             {item._err ? <div style={{ color: "#a11" }}>{item._err}</div> : null}
//           </div>
//         </div>
//       </div>
//     </div>
...
// /* ---------- Post item modal (create listing + edit/compress) ---------- */
// function PostItemModal({ onClose, onCreated, getAuthHeader }) {
...
//   // images: { file, previewUrl, editedFile, editedPreviewUrl }
...
//     } catch (e) {
//       setFormErr(e.message || "Failed to post item");
//     } finally {
//       setSubmitting(false);
//     }
//   };
...
//       <div className="m-modal" onClick={(e) => e.stopPropagation()}>
//         <div className="m-top">
//           <div className="m-title">Post Item</div>
//           <button className="m-close" onClick={onClose}>✕</button>
//         </div>
...
//                 accept="image/*"
//                 ref={fileInputRef}
//                  multiple onChange={onPickImages}
//                 style={{ display: "none" }}
//               />
...
//               </button>
//               </div>
//               {/* <div className="p-hint">Select images, then click Edit for crop/rotate/compress.</div> */}
//             </div>
...
//             <div className="p-field">
//               <label>Title</label>
//               <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g., iPhone 12 128GB" />
//             </div>

//             <div className="p-field">
//               <label>Price</label>
//               <input value={price} onChange={(e) => setPrice(e.target.value)} placeholder="e.g., 12000" inputMode="numeric" />
//             </div>
...
//               <input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g., Near campus gate" />
//             </div>
...
//             <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe condition, what's included..." />
//           </div>
...
export default function Marketplace() {
  ...
  // ---------- FIRST LOAD + WHEN SEARCH CHANGES OR VIEW ----------
  useEffect(() => {
    let mounted = true;
  ...
  const loadMore = async () => {
    try {
      ...
      const data = isMine
        ? await fetchMine({ limit: 12, cursor: nextCursor.cursor, id: nextCursor.id })
        : isSearching
        ? await fetchSearch({ q: debounced, limit: 12, cursor: nextCursor.cursor, id: nextCursor.id })
        : await fetchFeed({ limit: 12, cursor: nextCursor.cursor, id: nextCursor.id });
...
function ListingModal({
  ...
  const isMine =
    item?.seller?.id && loggedInUserId
      ? String(item.seller.id) === String(loggedInUserId)
      : false;
      ...
      <div className="m-modal" onClick={(e) => e.stopPropagation()}>
        <div className="m-top">
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            {item.seller?.profile ? (
              <img src={item.seller.profile} className="avatar-img" alt="profile" />
            ) : (
              <div className="avatar-fallback">
                {`${item.seller?.first_name?.[0] || ""}${item.seller?.last_name?.[0] || ""}`.toUpperCase()}
              </div>
            )}
            ...
            <div>
              ...
              >
                {`${item.seller?.first_name || ""} ${item.seller?.last_name || ""}`.trim() || "Unknown User"}
              </div>
        ...
        <div className="m-content">
          ...
          <div className="m-right">
            ...
            <div className="m-time">
              <TimeAgo timestamp={item.created_at} /> in <span>{item.location || "-"}</span>
            </div>
            ...
            {!isMine ? (
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                <button className="market-btn market-btn-secondary" onClick={() => onEditMine(item)}>
                  Edit
                </button>
                <button className="market-btn market-btn-secondary" onClick={() => onDeleteMine(item.id)}>
                  Delete
                </button>
              </div>
            )}
